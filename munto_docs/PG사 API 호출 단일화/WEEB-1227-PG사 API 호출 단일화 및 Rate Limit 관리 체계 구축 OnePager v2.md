...
| 동기 호출에서 대기(sleep) 시 유저 응답 지연 | 낮음 | 최대 대기 3초. Rate Limit 상황 자체가 드물고, 대기 없이 실패하는 것보다 나음 |
...
flowchart TD
    ...
    BS["BootpayService refund() / confirmReceipt() / ..."]
    ...
    TIMEOUT -- "No" --> ACQUIRE
    TIMEOUT -- "Yes" --> FORCE["한도 초과해도 호출 진행 (서비스 중단 방지)"]
    FORCE --> CALL

    CALL --> RESULT{"호출 결과"}
    RESULT -- "성공" --> DONE["결과 반환"]
    RESULT -- "Rate Limit 에러
    (9003 / 5xx)" --> RETRY{"재시도 가능? 
    (attempt < maxRetries)"}
...
private async executeWithRateLimit<T>(
  ...
  const { retryable = true, maxRetries = 2, retryDelayMs = 500 } = options ?? {};
  ...
  for (let attempt = 0; attempt <= (retryable ? maxRetries : 0); attempt++) {
    try {
      ...
      if (this.isRateLimitError(error) && retryable && attempt < maxRetries) {
        this.logger.warn(
          `PG Rate Limit 에러, 재시도 ${attempt + 1}/${maxRetries}: ${context}`
        );
...
| PG 호출 유형 | Rate Limit | 인라인 재시도 | 근거 |
| --- | --- | --- | --- |
| `refund` (환불) | O | 최대 2회, 500ms | 배치 집중 발생, 멱등성 보장 (`cancel_id`) |
| `requestSubscribeCardPayment` (정기결제) | O | 최대 2회, 500ms | 스케줄러 배치 |
| `destroyBillingKey` (빌링키 삭제) | O | 최대 2회, 500ms | 즉시 응답 불필요 |
| `confirmReceipt` (결제 확정) | O | 최대 2회, 500ms | 유저 대기 중이나 Rate Limit 상황은 드물어 최대 1초 추가 지연 허용 |
| `validateReceipt` / `getReceipt` (검증/조회) | O | 최대 2회, 500ms | 결제 플로우 내 |
| `getBillingKey` (빌링키 조회) | O | 최대 2회, 500ms | 결제 플로우 내 |
| `certificate` (본인인증) | O | **재시도 없음** | 멱등성 보장 불가, 실패 시 유저에게 즉시 안내 |
...
| 최대 대기 시간 | 3초 | 대기 초과 시 그대로 호출 (서비스 중단보다 나음) |
| 재시도 횟수 | 최대 2회 (초기 호출 포함 총 3회) | Rate Limit 순간 과부하 회복 대기 |
| 재시도 간격 | fixed 500ms | 유저 대기 시간 최소화 |
| 재시도 대상 에러 | RC_CANCEL_SERVER_ERROR(9003), PG 서버 5xx | 일시적 PG 과부하 |
