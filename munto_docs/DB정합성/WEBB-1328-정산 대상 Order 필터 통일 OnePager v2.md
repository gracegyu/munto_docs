...
5. **실패 상태 분리 처리** — "실패"라도 의미가 다름:
    - `FAILED_CANCEL_PAYMENT` (이탈자의 취소 환불 실패) → 정산 **제외** (모든 도메인)
    - `PARTIAL_REFUND_FAILED` (참여자의 얼리버드 환불 실패) → 정산 **포함** (소셜링만 발생). 멤버는 여전히 APPROVE 상태이므로 호스트는 참여 인원에 대해 정산받아야 함. 환불은 유저↔PG 이슈로 별도 해소
...
- **실패 상태 의미 구분**: `FAILED_CANCEL_PAYMENT`(이탈 멤버의 취소 환불 실패)와 `PARTIAL_REFUND_FAILED`(참여자의 얼리버드 환불 실패)는 PG 측면에선 둘 다 "실패"지만 정산 관점에선 완전히 다름. 전자는 멤버가 이미 이탈했으니 제외, 후자는 멤버가 참여했으니 포함(유저 환불 실패는 유저↔PG 별개 이슈). "실패 상태를 일괄 제외"하면 참여자 정산이 누락돼 호스트에 과소 지급 — 본 문서에서는 이 두 상태를 분리해서 취급
...
- **TOBE**: 필터에 `orderStatus IN (COMPLETED_PAYMENT, COMPLETED_CANCEL_PAYMENT) AND remainPrice > 0` 적용 + `settlementTargetCount = MIN(totalCount, Club.maximumMembers)` + `settlementTargetAmount = MIN(totalPaymentAmount, Club.price × settlementTargetCount)`. 클럽은 부분환불 없는 도메인이라 `PARTIAL_REFUNDED` 제외.

### 3.3 소셜링

- **ASIS**: Repository에서는 필터 없이 전체 Order를 로드하고 Service에서 상태별 분기 (`PARTIAL_REFUNDED` / `PARTIAL_REFUND_FAILED` / `remainPrice > 0`). Service 분기가 필터 역할을 겸해서 Repository·Service 책임이 섞여있고, `WAITING_PAYMENT`/`FAILED_PAYMENT`/`EXPIRED`/`FAILED_CANCEL_PAYMENT` 등을 구조적으로 배제하지 못해 remainPrice > 0이 남아있으면 정산에 끼어들 위험이 있음
- **TOBE**: Repository에서 `orderStatus IN (COMPLETED_PAYMENT, PARTIAL_REFUNDED, PARTIAL_REFUND_FAILED, COMPLETED_CANCEL_PAYMENT) AND remainPrice > 0` 필터를 명시. `PARTIAL_REFUND_FAILED`는 **유지** — 얼리버드 환불 실패는 멤버 이탈이 아니라 **참여자의 혜택 미지급**이라 호스트 정산과는 별개이며 멤버는 여전히 APPROVE 상태이므로 포함돼야 함. `FAILED_CANCEL_PAYMENT`(이탈자)는 여전히 제외.
- 기존 Service의 `PARTIAL_REFUND_FAILED` 분기는 Repository 필터와 정합성 맞춰 유지 (`PARTIAL_REFUNDED`와 동일 집계 로직)
...
```tsx
{
  orderStatus: {
    in: [
      ...
      OrderStatus.PARTIAL_REFUND_FAILED,
...
- **유저가 실지불한 금액(`remainPrice > 0`)이 있고 참여가 성립한 Order** (Invariant SE6)
- **실패 상태의 의미 분리**:
    - `FAILED_CANCEL_PAYMENT`: 이탈 멤버(CANCEL/KICK/REJECT)의 PG 환불 실패 → **정산 제외**. 재시도 스케줄러 해소 후 `COMPLETED_CANCEL_PAYMENT`로 전이되면 다음 주기에 복귀
    - `PARTIAL_REFUND_FAILED`: **참여자(APPROVE)**의 얼리버드 혜택 환불 실패 → **정산 포함**. 멤버는 참여했고 호스트는 그에 대해 정산받아야 함. 유저가 디스카운트를 PG로부터 못 받은 건 별개 이슈로 Slack 알림 후 수동/재시도 해소
- 결제 미완료(`WAITING_PAYMENT`, `FAILED_PAYMENT`, `EXPIRED`)는 대상 아님
- 환불 없는 취소(`COMPLETED_CANCEL_PAYMENT + remainPrice > 0`, 예: 소셜링 당일/3일 이내 취소, 클럽 ONETIME/GRACE_PERIOD 취소)도 포함 — **`improvements.md` §5 구현 전까지는 실제 데이터가 거의 없음**. §5 이후 자연 반영
- **도메인별 부분집합**: 부분환불이 있는 **소셜링만** `PARTIAL_REFUNDED` + `PARTIAL_REFUND_FAILED` 포함. 챌린지·클럽은 부분환불 발생 경로 없어서 제외.
...
```
(Repository) 필터: SE6(소셜링 4종) 공통 필터 + 기존 Socialing 조건

(Service) FOR EACH filtered Order:
  CASE orderStatus IN (PARTIAL_REFUNDED, PARTIAL_REFUND_FAILED):
    paymentAmount  += remainPrice
    discountAmount += refundPrice
    count          += 1
    IF PARTIAL_REFUND_FAILED: Slack 알림 (얼리버드 환불 실패 수동 확인 안내)
  그 외 (COMPLETED_PAYMENT, COMPLETED_CANCEL_PAYMENT):
    paymentAmount  += remainPrice
    count          += 1
...
`PARTIAL_REFUND_FAILED` 분기는 정산 집계 자체는 `PARTIAL_REFUNDED`와 동일하게 처리하되, 유저 환불이 실패한 건은 호스트 정산과 별개로 운영 이슈라 Slack 알림으로 분리.
...
| `PARTIAL_REFUNDED` | > 0 | ✅ (소셜링만) / ❌ (챌린지·클럽) | 얼리버드 — `remainPrice`/`refundPrice` 분리. 부분환불은 소셜링 얼리버드 전용, 챌린지·클럽은 발생 경로 없음 |
...
- **챌린지**: `COMPLETED_CANCEL_PAYMENT + remainPrice > 0` 건이 신규 포함되는지 + `PARTIAL_REFUNDED` Order는 SE6 필터에서 제외되는지 (애초에 발생하지 않지만 방어)
- **클럽**: 이중결제 2건 → `totalCount = 2`지만 `settlementTargetCount = MIN(2, maximumMembers)` / 정원 초과분 제외 / `GRACE_PERIOD` 이전 결제 Order의 정산 포함 여부 / `PARTIAL_REFUNDED` Order는 SE6 필터에서 제외 (발생 경로 없지만 방어)
...
| 항목 | 결정 | 근거 |
| --- | --- | --- |
| Order 필터 | `orderStatus IN (COMPLETED_PAYMENT, PARTIAL_REFUNDED, COMPLETED_CANCEL_PAYMENT) AND remainPrice > 0` — 단, `PARTIAL_REFUNDED`는 소셜링만(얼리버드 전용). 챌린지·클럽은 {`COMPLETED_PAYMENT`, `COMPLETED_CANCEL_PAYMENT`} | [정산 비즈니스 규칙 명세서](https://www.notion.so/348e2bc7639d80098f3bca59ea26c7a4?pvs=21)  Invariant SE6 |
...
| 챌린지 계산 전환 | `SUM(orderPrice)` → `SUM(remainPrice)` | "주문가" → "실지불 금액" 의미 정렬. 챌린지는 부분환불 없는 도메인이라 `PARTIAL_REFUNDED` 분기·`discountAmount` 불필요 |
...
| 항목 | 결정 | 근거 |
| --- | --- | --- |
| Order 필터 (도메인별) | 소셜링: `{COMPLETED_PAYMENT, PARTIAL_REFUNDED, PARTIAL_REFUND_FAILED, COMPLETED_CANCEL_PAYMENT}` / 챌린지·클럽: `{COMPLETED_PAYMENT, COMPLETED_CANCEL_PAYMENT}`. 모두 `AND remainPrice > 0` | `settlement.md` Invariant SE6 + 도메인 현실 반영 |
| 구조적 정합성 필터 (신설) | Order-도메인 FK 존재 + (CARD 전제) Payment 레코드 존재 | `settlement.md` Invariant SE7 신설 제안. `order-common.md` Invariant 1·2를 정산 쿼리가 재검증 |
| 정합성 위반 Order 처리 | 정산 제외 + Slack 알림 | 호스트 잘못 지급 방지. 일시적 위반은 수동 보정 후 다음 주기 복귀 |
| 챌린지 계산 전환 | `SUM(orderPrice)` → `SUM(remainPrice)` | "주문가" → "실지불 금액" 의미 정렬. 챌린지는 부분환불 없는 도메인이라 `PARTIAL_REFUNDED` 분기·`discountAmount` 불필요 |
| 클럽 정원 상한 | `MIN(totalCount, maximumMembers)` 신규 적용 | Invariant SE3. 소셜링과 일관성 |
| 소셜링 필터 위치 | Repository 레벨로 이동 | DB에서 필터링하여 불필요한 로드 방지 + Service 분기 단순화 |
| `PARTIAL_REFUND_FAILED` 처리 | 정산에 **포함** (소셜링 한정) + Slack 알림 | 멤버는 APPROVE로 참여했으므로 호스트 정산 대상. 얼리버드 환불 실패는 유저↔PG 이슈로 별도 해소 |
| `FAILED_CANCEL_PAYMENT` 처리 | 정산에서 제외 | 이탈 멤버이므로 정산 대상 아님. WEBB-1326 재시도 해소 후 `COMPLETED_CANCEL_PAYMENT` 전이돼 다음 주기 복귀 |
| 코스 | 본 PR에서 미변경 | 서비스 종료 예정. 별도 과제 |
| `improvements.md` §5 선행 | 불필요 | 필터가 `COMPLETED_CANCEL_PAYMENT`를 이미 포함하여 §5 구현 후 자동 반영 |
| 과거 데이터 보정 | 본 PR 범위 외 | 배포 전/후 운영 과제로 분리 |
...
| v1.0.1 | 26.04.23 | 김범진 | 챌린지, 클럽 부분환불 내용 제거 |
| v1.1.0 | 26.04.24 | 김범진 | 소셜링 부분환불 실패는 정산내역에 포함 |
