...
| Day 2 | 백엔드 — POST /v1/analytics/events endpoint, rate limiting, BullMQ 비동기 적재 |
...
| D6 | 백엔드 적재 동기성 | 202 Accepted + BullMQ 비동기 적재 | 동기 DB write | **A** | 202 Accepted + BullMQ 비동기 적재 | 동기 DB write | **A** | 응답 지연 최소화, 모바일 사용성 보호. 스키마 검증만 동기 |
...
post:
  ...
  responses:
    ...
    '500': { description: "서버 내부 에러 (BullMQ enqueue 실패 등)" }
```
