...
D5 선검증 — prod replica에서 고빈도 기존 행위 로그(조회·좋아요 성격 테이블, 결제는 저빈도 하한 표본)의 (user·event·ms) 동일 조합 충돌률을 측정해 자연 unique 가정 확정(충돌률이 무시 가능 수준이면 dedup 미도입 근거 확보) |
| Day 2 | 백엔드 — POST /v1/analytics/events endpoint, rate limiting, Bull 비동기 적재 |
| Day 3 | 모바일 — MuntoApiEventsOutput 구현(LogOutputInterface), 5종 화이트리스트, fire-and-forget 송신, 자연 키(socialing_id·order_id) 페이로드 매핑 |
| Day 4 | 통합 검증 — 5종 이벤트 dev 환경 end-to-end 테스트 |
...
| ⑦ 적재 | append-only (dedup 없음 — ms 단위로 자연 unique). 동기 응답 후 비동기 큐(Bull)로 DB write |
| ⑧ 집계 | 별도 view 없음 — 정합성 시트 갱신 시점에 raw 테이블에서 GROUP BY로 직접 조회 |
| ⑨ 정합성 비교 | `munto-analyze-paid-socialing` 스킬이 GROUP BY 조회 결과 × Meta Insights API로 diff 계산 |
...
| D4 | `platform` 저장 방식 | Device JOIN으로 조회 | denormalized 컬럼 보관 | **B** | platform(iOS/Android)이 본 프로젝트 정합성의 1차 축이라 모든 쿼리가 `GROUP BY platform` → 매번 JOIN 회피. `device_id=0`(x-device-id 누락) 케이스에도 헤더에서 산출한 platform 보존. `@@index([eventName, platform, createdAt])`로 핵심 쿼리 인덱스 활용. deviceType 불변이라 denormalize drift 없음. 상세는 §3.2 |
...
| D6 | 백엔드 적재 동기성 | 202 Accepted + BullMQ 비동기 적재 | 동기 DB write | **A** | 202 Accepted + Bull 비동기 적재 | 동기 DB write | **A** | 응답 지연 최소화, 모바일 사용성 보호. 스키마 검증만 동기 |
...
**채택 = A안**. 손실율을 운영 단계에서 모니터링하여 임계치(§8 정의, 10%) 초과 시 B안 전환 재검토.

**A안 모바일 세부 구현 결정**:

- **화이트리스트**: 5종(`complete_registration`·`view_paid_socialing`·`add_to_cart`·`begin_checkout`·`purchase`)만 통과
- **fire-and-forget**: `writeEvent()`는 `Future<void>` 즉시 반환, 백그라운드 송신. 실패 시 재시도 없이 폐기 + Crashlytics 실패 기록. 운영 손실율 임계치·측정 방법은 §8 단일 정의 참조
...
**채택 = B안**. platform은 정합성의 1차 축이라 매 쿼리 JOIN을 피하고, `device_id=0`(헤더 누락) 케이스에도 헤더 산출값으로 보존되며, 복합 인덱스로 핵심 집계가 인덱스만으로 처리된다. deviceType은 불변이라 denormalize로 인한 진실 분기 우려도 없음.
...
post:
  ...
  responses:
    '202':
      description: Accepted (스키마 검증 완료, Bull 큐 enqueue 성공. 동기 DB write 보장 안 함)
    '400': { description: "스키마 검증 실패" }
    '401': { description: "만료·무효 토큰 (모바일은 fire-and-forget으로 폐기, 재시도 없음). 5종 이벤트는 로그인·행위 성공 결과 시점에 발화하므로 사실상 발생 불가 — §8 손실율 제외" }
    '429': { description: "device 단위 rate limit 초과 (분당 200건)" }
    '500': { description: "서버 내부 에러 (Bull enqueue 실패 등)" }
```
...
model AnalyticsEvent {
  ...
  createdAt   DateTime @default(now()) @db.Timestamptz(3) @map("created_at")  // 백엔드 수신 시각, 행위 시각으로 간주
  ...
  platform    String?  @map("platform") @db.VarChar(10)  // 'IOS'|'ANDROID'|'UNKNOWN' (디바이스 헤더에서 산출)
  ...
  @@index([eventName, platform, createdAt])
...
- platform은 컬럼으로 적재되므로 `GROUP BY platform`에 JOIN 불필요. device의 다른 속성이 필요할 때만 `analytics_events ae JOIN devices d ON ae.device_id = d.id` 사용
...
**Munto 도메인 행위 → `event_name` 매핑** (정합성 시트에서 어느 행을 비교할지 기준):

| **Munto 도메인 행위** | **`event_name`** | **비고** |
| --- | --- | --- |
| 회원가입 완료 | `complete_registration` |  |
| 유료 소셜링 상세 조회 | `view_paid_socialing` |  |
| 좋아요(찜) | `add_to_cart` | 도메인 의미상 위시리스트(`add_to_wishlist`)에 가깝지만, 기존 Meta 이벤트 송신이 `add_to_cart`로 정착돼 있어 명칭을 그대로 유지. Munto에 "장바구니" 기능은 없으며 "좋아요" 행위를 가리킴 |
| 결제 시작 | `begin_checkout` |  |
| 구매 완료 | `purchase` |  |

> Jira 본문이 5종을 "회원가입·조회·**좋아요**·결제 시작·구매"로 적은 것과 본 문서의 `add_to_cart`는 동일 행위를 가리킨다 — 위 매핑이 단일 진실. §3.1·§5의 `add_to_cart`도 모두 "좋아요" 행위를 의미한다.
> 
...
- 송신 실패 시 재시도 없이 즉시 폐기. 실패 건은 Crashlytics에 기록(디버그 로그 포함)
- **운영 손실율 임계치 10%** 초과 시 재시도/큐 도입 재검토 (본 §8이 손실율 KPI의 단일 진실 위치)
- **손실율 측정 방법**: 모바일이 POST 응답(성공/실패)·네트워크 타임아웃을 인지할 수 있으므로, 실패 건을 Crashlytics에 기록하여 `손실율 = 실패 건수 / 송신 시도 건수`로 산출(일별/24h 윈도우). 외부 SDK 집계값은 신뢰도가 낮아 손실율 측정 기준으로 쓰지 않음
- **손실율 카운트 정의**: 네트워크 실패(timeout·연결 끊김)·`5xx`로 폐기된 건만 손실로 집계. `401`(인증)·`400`(검증)·`429`(rate limit) 등 4xx 응답은 송신 인프라 손실이 아니므로 손실율에서 **제외**(별도 집계)
- **401 발생 가능성**: 5종 이벤트는 모두 로그인·행위(회원가입·결제 등) 성공 결과 시점에 발화하므로 토큰 만료(401) 자체가 사실상 발생하지 않음
...
- 정합성 모니터링 시트 컬럼 매핑: 내부값은 본 스킬의 GROUP BY 조회 결과, Meta값은 Meta Event Manager, 같은 `(date, event_name, platform)` 를 기준으로 diff 계산
- DEVT-134 6번 작업("정합성 모니터링 시트 자동화")의 데이터 소스로 본 스킬의 GROUP BY 조회가 채택됨
...
| v1.0.1 | 26.05.20 | 김범진 |   • dedup 미도입 확정 — D5를 단일 진실로 두고 D6·Day 2의 dedup 표현 제거
  • API 응답 코드 401·500 추가, 401은 발생 불가·손실율 제외 명시
  • "view" 용어 통일 — §2·§9를 "스킬 GROUP BY 결과"로 치환 (§6 결정과 일치)
  • 이벤트 명명 정리 — §7.1에 도메인 행위→event_name 매핑 추가 (add_to_cart=좋아요)
  • 손실율 임계치 10%로 통일, Crashlytics 기반 측정 방법 명시 (§8 단일 정의)
  • 자연 unique 가정 검증을 Day 1 prod replica 선검증으로 이동 |
| v1.0.2 | 26.05.26 | 김범진 |   • 3.2 항목을 B안으로 수정 |
