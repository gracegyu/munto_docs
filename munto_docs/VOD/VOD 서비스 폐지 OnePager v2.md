...
| 5단계 (TBD) | **종료** | 별도 공지 후 결정. 최초 공지 시 종료 예정 시점 명시 필요 (약관 제10조 제4항) |
...
| 마이그레이션 | 1단계 | 없음 |
| 환경변수 추가 | 1단계 | 없음 |
| 외부 서비스 연동 변경 | 1단계 | 없음 (Bootpay 연동은 소셜링·클럽·챌린지 결제에서 계속 사용됨) |
| 데이터 보존 | 전 단계 공통 | 구매·결제 기록 물리 삭제 없음. DB 레코드 유지 (전자상거래법 제6조, 5년 보존 의무) |
| CourseStatus 전환 | 4단계 이후 | `ONSALE` → `CLOSED` 일괄 업데이트 스크립트 필요. 마이그레이션 아닌 데이터 패치 스크립트로 처리 |
| 시청 관련 데이터 처리 | 5단계 이후 | 대규모 VOD 데이터 soft-delete 처리 시 별도 마이그레이션 스크립트 필요. 실행 전 영향 범위 재검토 필요 |
