...
문토와 데이팅 서비스는 현재 외부 솔루션인 Sendbird에 채팅 기능을 의존하고 있어, 사용량 증가에 비례해 비용이 늘고 서비스 특성에 맞춘 기능 커스터마이징에 제약이 크다. 본 프로젝트는 이 외부 의존을 걷어내고 문토·데이팅이 공통으로 사용할 자체 채팅 서버를 구축하는 것을 목표로 한다. 채팅 서버는 두 서비스에서 독립적으로 확장·배포할 수 있도록 별도 MSA(Microservice Architecture)로 구성하며, munto-backend가 발급하는 통합 JWT Payload(문토 모노레포에 정의)를 채팅 서버와 데이팅 백엔드가 공통으로 검증해 단일 사용자 식별 체계로 인증을 처리한다. 이를 통해 Sendbird 대비 운영 비용을 절감하고, 푸시·금칙어·백오피스 등 문토 고유 정책을 자유롭게 반영할 수 있는 기반을 확보한다.

- 본 프로젝트에서는 다음과 같은 기능을 포함한다.
...
- 본 프로젝트에서는 다음과 같은 기능을 향후 계획에 포함한다. (요구사항 정의 완료, 구현 시점은 향후 결정)
  - **챌린지 채팅**: 챌린지 채팅은 인증하기, 인증률 표시 기능 등 별도 기능이 필요하다. 요구사항은 7.7.1에 정의되어 있으며, 챌린지 고도화 이후 구현 시점을 판단한다.
  - **클럽 채팅**: 클럽 채팅은 피드, 채널, 투표 기능 등 별도 기능이 필요하다. 요구사항은 7.7.2에 정의되어 있으며, 클럽 고도화 이후 구현 시점을 판단한다.
...
- **muntoUserId**: 채팅 서버·dating-backend·closer-notification이 공통으로 사용하는 사용자 식별자. 통합 JWT의 `sub` 클레임 값이자 dating DB `User.id`(FK 통합 구조)와 동일.
- **통합 JWT**: munto-backend가 `JWT_KEY`로 서명·발급하는 단일 JWT. payload의 `sub`(= muntoUserId)를 포함하며, dating-backend·채팅 서버가 `JWT_KEY`로 직접 검증. 2단계 토큰 교환 없음 (WEBB-1196).
...
- **munto-notification**: 문토 앱 및 임베드 데이팅 수신자 대상 FCM 푸시 발송 서버. Firebase 프로젝트 `munto-ecbbd` 사용.
- **closer-notification**: Closer 앱 수신자 전용 FCM 푸시 발송 신규 서버. Firebase 프로젝트 `closer-f4ac9` 사용. `munto-notification`과 **동일 코드베이스/이미지를 공유**하되 Firebase 프로젝트·credential만 환경변수로 분리하며, 기존 데이팅 AWS ECS 클러스터에 **별도 ECS 서비스로 배포**한다 (발송 경로·장애·credential 격리). 상세 설계 결정은 §6.1 푸시 서버 구성 참조.
...
- **사용자 상태 (3단계 모델)**: 해당 채팅방의 메시지를 실시간으로 수신할 수 있는지 여부를 기준으로 판단
  ...
  - **Offline (오프라인)**: Socket.IO 연결이 없는 상태. 연결이 끊기는 이유(앱 종료, 백그라운드 전환, 네트워크 끊김, 로그아웃 등)와 관계없이 오프라인으로 처리된다. **Push Worker가 노티 서버(munto-notification / closer-notification)를 통해 FCM/APNs 시스템 푸시를 발송**한다.
...
- **통합 JWT 기반 인증**: munto-backend가 발급하는 단일 JWT(`sub` = muntoUserId)를 채팅 서버와 dating-backend가 `JWT_KEY`로 직접 검증. 2단계 토큰 교환 없음 (WEBB-1196)
- **푸시 발송 격리**: Closer 수신자는 `closer-notification`(Firebase `closer-f4ac9`), 문토/임베드 수신자는 `munto-notification`(Firebase `munto-ecbbd`)으로 발송 경로 완전 분리
...
graph TB
    ...
    subgraph PushSvc["푸시 서비스"]
        MuntoNotif["munto-notification\n(munto-ecbbd)"]
        CloserNotif["closer-notification\n(closer-f4ac9)"]
        FCM["FCM / APNs"]
    end
    ...
    %% 채팅 서버 → 푸시
    Chat -->|"문토/임베드 수신자 푸시"| MuntoNotif
    Chat -->|"Closer 수신자 푸시"| CloserNotif
    MuntoNotif & CloserNotif --> FCM
...
**오프라인 대응 범위**: 앱은 네트워크 없이 실행이 어려운 특성상 완전 오프라인 동작은 지원하지 않는다. 소켓 일시 끊김 또는 전송 실패 시 FAILED 상태로 로컬 저장하고, 사용자가 명시적으로 재전송 버튼을 눌러 재시도한다.

### 2.2.2 채팅 서버 내부 구조 (munto-chat-backend)
...
graph TB
    ...
    subgraph ChatBackend["munto-chat-backend"]
        subgraph RestModule["REST API 모듈"]
            RoomAPI["채팅방 API\n생성 · 조회 · 나가기\n참여자 · 사진 · 공지 목록"]
            MsgAPI["메시지 API\n히스토리 조회\n메시지 삭제"]
            UploadAPI["업로드 API\nPresigned URL 발급"]
            AdminAPI["백오피스 API\n채팅 내역 조회\n금칙어 · 차단 관리"]
        ...
        subgraph WSModule["Socket.IO 모듈"]
        ...
        subgraph PushWorker["Push Worker (별도 프로세스)"]
            ...
            Router["수신자 앱 타입 분기\nmuntoUserId → 앱 판별"]
            MuntoNotiClient["munto-notification 호출"]
            CloserNotiClient["closer-notification 호출"]
        end

        RedisAdapter["Redis Adapter\n(Socket.IO 다중 서버 Pub/Sub)"]
    end
    ...
    RestModule --> PG
    WSModule --> PG
    WSModule --> RedisAdapter
    RedisAdapter <--> Redis

    MsgEvt -->|"Redis Streams 발행"| Streams
    Streams --> Router
    Router --> MuntoNotiClient & CloserNotiClient
    MuntoNotiClient --> MuntoNotif
    CloserNotiClient --> CloserNotif
...
| 컴포넌트          | 역할                                                                    |
| ------------- | --------------------------------------------------------------------- |
| REST API 모듈   | 채팅방 CRUD, 메시지 히스토리, 이미지 업로드 URL 발급, 백오피스 기능                           |
| Socket.IO 모듈  | JWT 검증 후 실시간 이벤트 처리. 채팅방 입퇴장, 메시지, 읽음, 타이핑, Heartbeat                 |
| Redis Adapter | 다중 Socket.IO 서버 간 메시지 동기화 (Pub/Sub). 사용자 상태(Active/Idle/Offline) 관리   |
| Push Worker   | 별도 프로세스. Redis Streams에서 푸시 이벤트 소비 → 수신자 앱 타입 판별 → notification 서버 위임 |
| PostgreSQL    | 채팅방, 메시지, 참여자, 읽음 상태 등 채팅 데이터 영구 저장                                   |
| Redis         | Socket.IO Pub/Sub, 사용자 온라인 상태, Push Worker 이벤트 큐(Streams)             |
...
3. **푸시 알림 발송 (Hybrid 구조, 3단계 상태 모델)**
  - 채팅 서버에서 사용자 상태(Active/Idle/Offline)를 판단하여 푸시 발송 여부 결정
    ...
    - Offline: Redis Streams로 푸시 이벤트 발행 → Push Worker → 노티 서버 → FCM/APNs 발송
...
- 차단 대상 메시지 자동 차단 (금칙어, 외부 링크, 부적절한 이미지)
- 신고 기능 (사용자 신고 + 자동 신고)
- 사용자 차단 기능
- DM 어뷰징 방지 (Rate Limiting)

**클럽/챌린지 특수 기능 (향후 구현 예정)**
...
- **연령대**: 주로 20-30대
- **주요 기능**: 데이팅 DM
- **클라이언트**: Closer 앱 (RN/Expo) — `@munto/chat-sdk` 사용
- **전환기 병행**: 문토 임베드 데이팅(`dating-mobile`) 사용자도 동일 채팅 서버 공유, 전환 유도 후 sunset

**관리자 (Administrators)**

- **자격 요건**: 문토 운영팀
- **주요 기능**: 채팅 내역 조회 및 모니터링, 신고 처리, 금칙어 관리, 사용자 차단
...
- `sub`: muntoUserId (사용자 식별자, 채팅 서버와 dating-backend 공통 사용)
...
- 1순위(데이팅 DM)는 Closer(RN 독립 앱)로 분리되어 있어 기획 의존성 없이 독립적으로 진행 가능. Closer는 출시 시점부터 자체 채팅 서버만 사용 (Sendbird 미적용)
- 채팅 목록 기획은 Phase 2(데이팅 DM 전환) 진행 중에 확정될 예정

## **2.7 Apportioning of Requirements (단계별 요구사항)**

본 프로젝트의 모든 기능 요구사항은 **P1 (프로젝트에 반드시 포함되어야 하는 핵심 기능)**으로 분류된다.
...
| 순위         | 대상               | 확정 여부    | 이유                                    |
| ---------- | ---------------- | -------- | ------------------------------------- |
| **1순위**    | 데이팅 DM           | 확정       | Closer RN 독립 앱으로 분리, 처음부터 자체 채팅 서버 사용 |
| **2순위 이후** | 소셜링/클럽/챌린지/문토 DM | 기획 확정 필요 | 채팅 목록 정렬 방식에 따라 순서 결정                 |


**1순위: 데이팅 DM (확정)**

- Closer(RN 독립 앱)로 분리되어 독립적 전환 가능. Closer는 출시 시점부터 Sendbird 없이 자체 채팅 서버만 사용
- 문토 앱과 완전히 분리된 앱이므로 이중 유지 부담 최소
- 임베드 데이팅(`dating-mobile`)의 기존 진행 중인 채팅방: Sendbird → 자체 채팅 서버 마이그레이션
- 30일 만료 정책으로 만료된 채팅방은 마이그레이션 제외 (자연 소멸)
- 데이팅 서비스 확장 시 Sendbird 비용 증가 방지
...
- 1:1 구조로 마이그레이션 용이
- Closer는 독립 앱이므로 문토 앱 영향 없이 전환 가능
...
단계적 마이그레이션을 통해 리스크를 분산하고 점진적 비용 절감을 달성한다.


| Phase       | 내용                  | 기간 (예상) | 비고               |
| ----------- | ------------------- | ------- | ---------------- |
| **Phase 1** | 자체 채팅 서버 구축 및 테스트   | 4~6주    | 공통 인프라           |
| **Phase 2** | 데이팅 DM 전환 (1순위)     | 2~4주    | Closer 독립 앱, 파일럿 |
...
```
1. Closer 앱(RN)에 @munto/chat-sdk 연동 — 출시 시점부터 자체 채팅 서버만 사용
2. 임베드 데이팅(dating-mobile) 진행중 채팅방 메시지 마이그레이션 (Sendbird Export API)
3. 신규 임베드 데이팅 채팅방 → 자체 서버 전환
4. 30일 후 만료된 채팅방 자연 소멸 (마이그레이션 대상 제외)
5. 임베드 데이팅에서 Sendbird SDK 제거 (sunset 단계에서)
6. 안정화 모니터링 및 이슈 대응
```
...
| TypeScript (웹) | `@sendbird/chat` | `@munto/chat-sdk` (현재 범위 제외 — 웹 채팅 필요 시점에 재검토) |
...
- **사용자 식별**: `sub` 클레임 = muntoUserId (단일 필드 사용)
...
채팅 SDK(`munto_chat_sdk` / `@munto/chat-sdk`)가 기기 내에 유지하는 로컬 DB 스키마이다. 상세 테이블 정의 및 삭제 정책은 위 문서를 참조하세요.
...
- 플랫폼별 구현: Flutter → Drift/Isar, React Native(Closer) → SQLite/MMKV
...
- TBD — 담당: @{백엔드 담당자}, 결정 시점: Phase 1(채팅 서버 구축) 완료 후 벤치마크 측정, Jira: CHAT-XXX. 측정 결과에 따라 최종 SLA 수치 확정
...
**현재 선택: Redis Streams + Push Worker (비동기 발송)**

실패 처리 및 재시도를 고려하여 Redis Streams 기반의 비동기 구조를 채택한다. 기존 마케팅 푸시 서버(Kafka 기반)와 분리하여 채팅 전용 Push Worker를 구성한다. Push Worker는 FCM/APNs를 직접 호출하지 않고 **푸시 서버(노티 서버)에 발송을 위임**하며, 실제 FCM/APNs 발송과 device token·credential 관리는 노티 서버가 담당한다.
...
[Push Worker] ← 별도 프로세스 (현재 chat-backend 같은 레포)
 ...
 ├─ 노티 서버 호출 (HTTP 위임)
 │     ├─ 문토/임베드 → munto-notification (munto-ecbbd)
 │     └─ Closer      → closer-notification (closer-f4ac9)
 ├─ 실패 시 재시도 (최대 3회, 지수 백오프)
 └─ 최종 실패 → 로그 기록 (DLQ)

[노티 서버 (푸시 서버)] ← FCM/APNs 실제 발송 말단
 └─ device token 조회 → FCM/APNs 발송
```

**푸시 서버(노티 서버) 구성:**

채팅 푸시의 실제 발송 말단은 앱별로 분리된 두 노티 서버다. Closer는 독자 Firebase 프로젝트(`closer-f4ac9`)와 별도 device token 풀을 가지므로, 문토/임베드용 `munto-notification`과 발송 경로를 분리한다.

| 노티 서버 | 대상 수신자 | Firebase 프로젝트 | 배포 |
| --------------------- | ----------------- | ----------- | ----------------------------------- |
| `munto-notification`  | 문토 앱 · 임베드 데이팅 | `munto-ecbbd` | 기존 운영 서비스 |
| `closer-notification` | Closer 앱         | `closer-f4ac9` | 데이팅 AWS ECS 클러스터에 별도 ECS 서비스로 신규 배포 |

> **설계 결정 — 신규 분리 + 코드 공유**: `closer-notification`은 `munto-notification`과 **동일 코드베이스/이미지를 공유**하되, Firebase 프로젝트·credential을 **환경변수로 분리**하고 **독립 ECS 서비스로 배포**한다. 이로써 ① 발송 경로·장애·credential 격리(§2.1 푸시 발송 격리 원칙)와 ② 코드 중복 제거를 동시에 확보한다. 단일 서버에 두 Firebase 프로젝트를 통합하는 방식은 격리 원칙을 깨고 blast radius·credential 권한 범위를 넓히므로 채택하지 않는다.
...
| **Push Worker** | Redis Streams 소비, 수신자 앱 타입 판별, 노티 서버 호출(위임), 재시도 처리 |
| **노티 서버**(munto-notification / closer-notification) | device token 관리, FCM/APNs 실제 발송 |
...
이미지 메시지에 포함된 부적절한 콘텐츠를 자동으로 감지하고 차단하는 기능이다.
...
- 관리자(문토봇) 이미지는 부적절한 이미지 감지 검사 제외
...
다음 경우 시스템이 자동으로 신고를 생성한다:

- 금칙어 포함 메시지 (7.6.3 참조)
- 외부 링크 포함 메시지 (7.6.4 참조)
- 부적절한 이미지 (7.6.5 참조)

자동 신고된 메시지는 백오피스에서 검토 대상으로 등록된다.
...
사용자가 특정 사용자를 차단하여 더 이상 메시지를 주고받지 못하도록 하는 기능이다.
...
> TBD — 담당: PM, 결정 시점: Phase 2(문토 DM 전환) 착수 전, Jira: CHAT-YYY. 아래 수치는 "DM 수신 후 소셜링 참여 전환" 데이터 분석 결과에 따라 확정한다.
...
백오피스에서 사용자를 차단하거나 제재 조치를 취하는 기능이다.
