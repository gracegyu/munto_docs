...
분류: SRS
작성자: 김범진
최초 작성일: 2025년 11월 11일 오후 2:07
최근 수정일: 2026년 1월 23일 오후 2:09
문서 상태: Active
생성 일시: 2025년 11월 11일 오후 2:07
최종 편집자: 김세현
...
- 본 프로젝트에서는 다음과 같은 기능을 포함합니다.
    ...
    - **푸시 알림**: 채팅 메시지 수신 시 푸시 알림 발송 기능 (기존 Kafka 인프라 활용)
...
- **사용자 상태 (3단계 모델)**: 해당 채팅방의 메시지를 실시간으로 수신할 수 있는지 여부를 기준으로 판단
    ...
    - **Offline (오프라인)**: Socket.IO 연결이 없거나, 마지막 heartbeat 이후 180초 경과한 상태. 연결이 끊기는 이유(앱 종료, 백그라운드 전환, 네트워크 끊김, 로그아웃 등)와 관계없이 오프라인으로 처리된다. **Kafka를 통해 FCM/APNs 시스템 푸시를 발송**한다.
...
- API & ERD : [https://github.com/Munto-dev/chat-docs](https://github.com/Munto-dev/chat-docs)
- POC 진행 후 추가 예정
...
채팅 서비스 내재화의 **비즈니스 가치와 전략적 의사결정**에 필요한 정보를 확인합니다.

| **추천 섹션** | **내용** |
| --- | --- |
| **1.1~1.2** (목표 및 범위) | 프로젝트 목표, Sendbird 대체 배경, 비용 절감 효과 |
| **2.1~2.4** (시스템 개요) | 전체 아키텍처, 주요 기능 개요 |
| **2.8** (하위 호환성) ⭐ | **마이그레이션 전략**, Phase별 전환 계획, 비용 절감 로드맵, 리스크 관리 |

**핵심 확인 포인트:**

- 2.8.3: Phase별 마이그레이션 단계 및 일정
- 2.8.5: 리스크 관리 및 롤백 전략
- 2.8.6: 성공 기준 (기능 완성도, 비용 절감 등)

### **1.6.2 기획자 & 디자이너**

기능 정의, 사용자 플로우, **UX 설계에 필요한 상세 요구사항**을 확인합니다.

| **추천 섹션** | **내용** |
| --- | --- |
| **1.4** (용어 정의) | Active/Idle/Offline 상태, 채팅 SDK, 문토봇 등 핵심 용어 |
| **2.3~2.4** (동작방식) | 채팅방 생성, 메시지 전송/수신, 푸시 알림 플로우 |
| **7.1** (인증 및 연결) | Socket.IO 연결 관리, 재연결 전략 |
| **7.2** (채팅방 관리) ⭐ | 채팅방 생성/조회/나가기, 참여자 목록, 사진/공지 모아보기 |
| **7.3** (메시지 전송) ⭐ | 텍스트/이미지 전송, 사진 모아보내기, 공지 설정 |
| **7.4** (읽음 처리) | 읽음 처리, 읽지 않은 메시지 수, 입력 중 표시 |
| **7.5** (푸시 알림) | 푸시 발송 조건, 채팅방별 알림 설정 |
| **7.6** (안전 기능) | 금칙어 필터링, 외부 링크 차단, 신고/차단 |
| **7.7** (클럽/챌린지) | 향후 확장 기능 (인증, 피드, 투표) |

**핵심 확인 포인트:**

- 7.2.1: 채팅방 타입별 생성 조건 (소셜링/DM/데이팅 DM)
- 7.3.2: 사진 모아보내기 (최대 10장, 배열 형태)
- 7.4.3: 사용자 상태 3단계 모델 (푸시 분기 로직)
- 7.6.2: 차단 메시지 처리 정책 (블라인드, 자동 신고)

### **1.6.3 개발자**

시스템 아키텍처, API 설계, 데이터 모델 등 **기술적 구현 사항을 완벽히 이해하고 개발**할 수 있도록 작성되었습니다.

| **추천 섹션** | **내용** |
| --- | --- |
| **3장** (환경) | 하드웨어/소프트웨어 환경, 기술 스택 |
| **4장** (외부 인터페이스) | JWT 인증, Socket.IO 이벤트, API 표준화 |
| **5장** (성능 요구사항) | 동시 접속, 처리량, Socket.IO 연결 제한 |
| **6장** (비기능 요구사항) | 안전성, 보안, 가용성, 데이터 일관성 |
| **7장** (기능 요구사항) | 전체 기능 상세 명세 |

**별도 문서 참조:**

- [ERD 문서](https://file+.vscode-resource.vscode-cdn.net/Users/munto/Projects/document/chat-docs/erd.md): 데이터베이스 스키마
- [API 문서](https://file+.vscode-resource.vscode-cdn.net/Users/munto/Projects/document/chat-docs/api.yaml): REST API 명세
- [Socket.IO Events 문서](https://file+.vscode-resource.vscode-cdn.net/Users/munto/Projects/document/chat-docs/websocket-events.md): 실시간 통신 이벤트

### **1.6.4 운영팀**

신고/차단 처리, 사용자 안전 정책, **백오피스 관리 기능**을 파악합니다.

| **추천 섹션** | **내용** |
| --- | --- |
| **6.1~6.2** (안전성/보안) | 데이터 손실 방지, 사용자 보호, 인증/접근 제어 |
| **6.5** (비즈니스 규칙) | 채팅방/메시지 규칙, 데이터 보관 기간 |
| **7.6** (안전 기능) | 금칙어, 외부 링크 차단, 신고/차단 처리 |
| **7.8** (관리자 기능) ⭐ | 채팅 내역 조회, 통계, 사용자 제재, 금칙어 관리 |

**핵심 확인 포인트:**

- 7.6.2: 차단 메시지 처리 정책 (블라인드, 자동 신고)
- 7.8.1: 접근 권한 및 감사 로그
- 7.8.4: 사용자 차단/제재 처리 (경고, 일시 정지, 영구 정지)
...
graph TB
    ...
    ChatWS1 -->|푸시 이벤트 발행| Kafka
    ChatWS2 -->|푸시 이벤트 발행| Kafka
    Kafka -->|푸시 이벤트 소비| PushServer
...
- **채팅 서버 (MSA)**
    ...
    - Kafka: 푸시 알림 이벤트 발행 (기존 Kafka 인프라 활용)
- **푸시 서비스**
    - Kafka를 통해 푸시 이벤트를 수신하여 FCM/APNs로 푸시 알림 발송
    - 기존 문토 푸시 서비스 인프라 활용
...
1. **메시지 전송**
    ...
    - Kafka를 통해 푸시 알림 이벤트 발행 (오프라인 사용자 대상)
2. **메시지 수신**
...
3. **푸시 알림 발송 (Hybrid 구조, 3단계 상태 모델)**
    ...
    - Offline: Kafka를 통해 푸시 이벤트 발행 → 푸시 서버에서 FCM/APNs 발송
...
- Kafka를 통한 푸시 이벤트 발행 (기존 Kafka 인프라 활용)
...
[채팅 서버 구축을 위한 유저 통합 전략 검토 보고서](https://www.notion.so/2e0e2bc7639d809781c2fd387fbfe3eb?pvs=21)
...
| **2순위 이후** | 소셜링/클럽/챌린지/문토 DM | 기획 확정 필요 | 채팅 목록 밍글 방식에 따라 순서 결정 |
...
- **메시지 큐**
    - AWS MSK (Managed Streaming for Apache Kafka) 또는 기존 Kafka 인프라 활용
...
- **메시지 큐**:
    - Kafka (기존 문토 Kafka 인프라 활용)
    - kafkajs (Kafka 클라이언트 라이브러리)
- **Mobile SDK (Flutter)**:
    - 패키지명: munto_chat_sdk
    - Flutter 3.x+
...
- [Backend](https://github.com/Munto-dev/chat-backend)
- [Mobile SDK](https://github.com/Munto-dev/munto-mobile-chat-sdk)
- [Web SDK](https://github.com/Munto-dev/munto-frontend-chat-sdk)
...
https://github.com/Munto-dev/chat-docs/blob/main/api.yaml
...
https://github.com/Munto-dev/chat-docs/blob/main/websocket-events.mdjwt-integration-api.yaml
...
- 재시도 실패 시 사용자에게 명확한 오류 메시지 표시 및 전송 실패 상태 표시
- 사용자는 전송 실패한 메시지를 수동으로 재전송할 수 있어야 함
- 전송 실패한 메시지는 클라이언트 로컬 스토리지에 임시 저장
- 서버 복구 또는 재연결 시 저장된 메시지를 자동으로 재전송
- 재전송 성공 시 로컬 저장된 메시지 삭제
...
- 좀비 Socket.IO 연결 방지: 브라우저의 메모리 세이브 모드 등으로 인해 클라이언트가 응답하지 않는 좀비 연결을 방지하기 위한 Ping/Pong 메커니즘 구현
- 서버에서 주기적으로 클라이언트에게 Ping 메시지 전송
- 클라이언트는 Ping 메시지 수신 시 즉시 Pong 응답 전송
- 일정 시간(예: 30초) 동안 Pong 응답이 없으면 서버에서 해당 연결을 강제로 종료
- 연결 종료 시 사용자 상태를 오프라인으로 변경 및 리소스 정리
- Socket.IO의 자동 Ping-Pong 메커니즘과 별도로 커스텀 Ping/Pong 이벤트 구현 (웹 브라우저 메모리 세이브 모드 대응)
    - 서버: 주기적으로 `ping` 이벤트 전송, 응답 없으면 연결 종료
    - 클라이언트 (SDK): `ping` 이벤트 수신 시 `pong` 응답 자동 처리
...
채팅 푸시 알림은 마케팅/공지형 대량 푸시와 성격이 다르며, **즉시성**이 가장 중요한 요구사항이다. 본 시스템은 **Hybrid 구조**를 채택하여 즉시성 SLA를 보장하면서 기존 Kafka 인프라를 재사용한다.

**채팅 푸시 vs 대량 푸시 특성 비교:**

| **구분** | **채팅 푸시** | **마케팅/공지형 푸시** |
| --- | --- | --- |
| 발생 빈도 | 낮음 (1:1 또는 소규모) | 높음 (대량 발송) |
| 즉시성 요구 | **매우 높음** | 낮음 (분 단위 허용) |
...
| **Offline** | Socket.IO 미연결 or heartbeat 180초 경과 | - | FCM/APNs 시스템 푸시 |

**역할 분리:**

| **컴포넌트** | **책임** |
| --- | --- |
| **Chat Server** | 3단계 상태 판단 (Redis 조회), **즉시성 SLA 직접 책임**, 푸시 발송 여부 결정 |
| **Kafka** | Offline 사용자 대상 푸시 이벤트 전달 파이프라인 (비동기) |
| **Push Server** | FCM/APNs 발송 수행, 재시도 처리 |
...
6. Kafka는 **푸시 이벤트 전달을 위한 비동기 파이프라인**으로만 사용한다.

**Kafka 선택 근거:**

SQS, EventBridge 등 다른 전달 수단도 기술적으로 가능하나, 본 프로젝트에서는:

- 기존 Kafka 인프라를 이미 운영 중이며 검증됨
- 대량 푸시와 채팅 푸시를 동일 파이프라인 내에서 토픽 분리로 운영 가능
- 추가 인프라 도입에 따른 전환 리스크 최소화
- 푸시 서버가 이미 Kafka Consumer로 구현되어 있음

따라서 **메시지 전달 수단은 교체 가능한 구현 요소**로 간주하며, 현재는 Kafka를 재사용하는 것이 비용·운영 안정성·전환 리스크 측면에서 합리적이다.

**장점:**

- 즉시성 SLA는 Chat Server가 직접 책임 (Kafka 지연과 무관)
- 기존 인프라 재사용으로 비용 및 운영 안정성 확보
- Active/Idle 사용자는 Kafka를 거치지 않으므로 Kafka 부하 감소
- 대량 푸시와 채팅 푸시를 별도 Kafka 토픽으로 분리하여 장애 격리 가능
...
- **데이터베이스 스키마**: [ERD 문서](https://file+.vscode-resource.vscode-cdn.net/Users/munto/Projects/document/chat-docs/erd.md) 참조
- **실시간 통신 이벤트**: [Socket.IO Events 문서](https://file+.vscode-resource.vscode-cdn.net/Users/munto/Projects/document/chat-docs/Socket.IO-events.md) 참조
...
- PostgreSQL (RDB), Kafka (비동기 처리) 사용
...
상세한 JWT 통합 계획은 [JWT 통합 계획서](https://file+.vscode-resource.vscode-cdn.net/Users/munto/Projects/document/chat-docs/jwt-integration-plan.md)를 참조하세요.
...
- Socket.IO의 자동 Ping-Pong 메커니즘 사용
- 서버와 클라이언트 간 주기적으로 연결 상태 확인
- 일정 시간 동안 응답이 없으면 연결 종료 처리
- 연결 타임아웃: 60초 (기본값)

**웹 채팅 메모리 세이브 모드 대응:**

- 웹 브라우저의 메모리 세이브 모드에서는 메시지 수신이 보장되지 않을 수 있음
- 별도의 핑퐁(Ping-Pong) 헬스체크를 통해 연결 상태 확인
- 헬스체크가 끊어지면 연결을 즉시 종료하여 불안정한 연결 상태 방지
- 헬스체크 주기: 30초 (구현 시 조정 가능)

**연결 상태 관리:**

- 연결 수립 시: 사용자를 온라인 상태로 변경
- 연결 종료 시: 사용자를 오프라인 상태로 변경
- 다중 기기 지원: 각 기기별로 독립적인 Socket.IO 연결 관리
...
7. Kafka를 통해 푸시 알림 이벤트 발행 (오프라인 사용자 대상)
...
2. 클라이언트에서 각 이미지 전처리:
    ...
    - 각 이미지 크기 확인 및 리사이징 (최대 10MB 제한)
    - 필요 시 각 이미지 리사이징하여 크기 조정
...
8. Kafka를 통해 푸시 알림 이벤트 발행 (오프라인 사용자 대상)
...
- 이미지 리사이징: 클라이언트에서 필요 시 리사이징하여 전송
...
**사용자 상태 저장 (Redis Hash):**

```
Key: user_status:{userId}
Value: {
  status: 'ACTIVE' | 'IDLE' | 'OFFLINE',
  activeRoomId: number | null,    // 현재 보고 있는 채팅방 ID
  lastActivityAt: number,         // 마지막 활동 시간 (Unix time ms)
  lastHeartbeatAt: number         // 마지막 heartbeat 시간 (Unix time ms)
}

조회: HGETALL user_status:{userId}
갱신: HMSET user_status:{userId} status ACTIVE activeRoomId 123 ...
삭제: DEL user_status:{userId}

```

- Hash 자료구조로 사용자별 상세 상태 관리
- 상태 조회/갱신 모두 O(1) 성능
- TTL 설정: Offline 상태 시 24시간 후 자동 삭제

**상태 전이 조건:**

- **[연결 시] → IDLE**: `activeRoomId = null`
- **IDLE → ACTIVE**: `join_room` 이벤트 수신, `activeRoomId = 해당 채팅방 ID`
- **ACTIVE → ACTIVE**: 다른 채팅방 `join_room`, `activeRoomId = 새 채팅방 ID` (기존 방은 자동 비활성)
- **ACTIVE → IDLE**: 채팅 목록으로 이동 (`activeRoomId = null`) 또는 `lastActivityAt` 60초 경과
- **IDLE/ACTIVE → OFFLINE**: Socket.IO 연결 종료 또는 `lastHeartbeatAt` 180초 경과
- **OFFLINE → IDLE**: Socket.IO 재연결

**상태 관리 프로세스:**

1. **Socket.IO 연결 시:**
    - Redis에 사용자 상태 생성: `status = IDLE`, `activeRoomId = null`
    - `lastActivityAt`, `lastHeartbeatAt` 현재 시간으로 설정
2. **`join_room` 이벤트 수신 시:**
    - `status = ACTIVE`, `activeRoomId = roomId`
    - `lastActivityAt` 갱신
    - 이전 activeRoomId가 있으면 자동으로 해당 방은 비활성 처리
3. **채팅 목록 이동 시 (클라이언트에서 알림):**
    - `status = IDLE`, `activeRoomId = null`
    - 별도 이벤트 없이 다른 채팅방 `join_room` 시 자동 처리
4. **`ping` (heartbeat) 이벤트 수신 시:**
    - `lastHeartbeatAt` 갱신
    - 클라이언트는 30초마다 `ping` 이벤트 전송
5. **Socket.IO 연결 종료 시:**
    - `status = OFFLINE`, `activeRoomId = null`
    - 상태 데이터는 바로 삭제하지 않고 24시간 TTL 설정
    - 재연결 시 이전 상태 정보 참조 가능
    - 24시간 내 재연결 없으면 자동 삭제
6. **타임아웃 체크 (서버 주기적 실행):**
    - `lastHeartbeatAt` 180초 경과 시 → OFFLINE 처리
    - `lastActivityAt` 60초 경과 시 → ACTIVE → IDLE 전환
...
- 사용자가 여러 디바이스에서 접속해도 `user_status:{userId}`는 하나만 유지
- 가장 최근에 활동한 디바이스 기준으로 상태 관리
- 하나라도 Active/Idle이면 시스템 푸시 미발송

**메시지 전송 시 상태 확인 및 푸시 분기:**

1. 새 메시지 발생 시 채팅방 참여자 목록 조회 (DB)
2. 각 참여자별로 Redis 조회: `HGETALL user_status:{userId}`
3. 상태별 처리:
    - **Active (해당 roomId)**: Socket.IO로 `new_message` 전송, 푸시 없음
    - **Active (다른 roomId) / Idle**: Socket.IO로 `new_message` 전송, 클라이언트에서 로컬 푸시
    - **Offline**: Kafka로 푸시 이벤트 발행 → FCM/APNs 시스템 푸시
...
2. 푸시 알림 이벤트 발행:
    - 채팅 서버에서 Kafka로 푸시 이벤트 발행 (기존 Kafka 인프라 활용)
    - 이벤트에 포함되는 정보: 수신자 ID, 발신자 정보, 메시지 내용, 채팅방 ID 등
3. 푸시 서버에서 이벤트 소비:
    - 기존 푸시 서버가 Kafka에서 푸시 이벤트 수신
...
- **오프라인 사용자**: Socket.IO 연결이 없는 사용자 → Kafka 경유 FCM/APNs 푸시
...
4. 오프라인 사용자만: Kafka로 푸시 이벤트 발행 → FCM/APNs 시스템 푸시

**장점:**

- Kafka/FCM 부하 감소: 온라인 사용자 비율만큼 푸시 메시지 감소 (약 30% 절감)
- 지연 시간 개선: 온라인 사용자는 즉시 수신 (Kafka 경유 ~500ms → 직접 ~10ms)
...
클럽 채팅방과 챌린지 채팅방은 일반 소셜링 채팅방과 달리 특수 기능을 제공합니다. 데이터는 문토 백엔드에서 관리하며, 채팅 서버는 메시지 표시 및 연동 역할을 수행합니다.
