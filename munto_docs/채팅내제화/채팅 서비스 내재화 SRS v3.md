...
- **사용자 상태 (3단계 모델)**: 해당 채팅방의 메시지를 실시간으로 수신할 수 있는지 여부를 기준으로 판단
    ...
    - **Offline (오프라인)**: Socket.IO 연결이 없는 상태. 연결이 끊기는 이유(앱 종료, 백그라운드 전환, 네트워크 끊김, 로그아웃 등)와 관계없이 오프라인으로 처리된다. **Push Worker를 통해 FCM/APNs 시스템 푸시를 발송**한다.
...
- API: [swagger.yaml](https://www.notion.so/munto/swagger.yaml)
- ERD: [ERD](https://www.notion.so/munto/erd.md)
- 소켓 이벤트: [socket-events](https://www.notion.so/munto/socket-events.md)
...
현재 문토 백엔드와 데이팅 백엔드는 별도의 데이터베이스를 사용하지만 문토 사용자 ID를 기준으로 동일한 사용자 ID 체계를 사용하고 있다. 따라서 채팅서버도 문토 사용자 ID를 기준으로 동일한 사용자 식별 체계를 사용한다.
...
https://github.com/Munto-dev/chat-document/blob/development/database/erd.md

https://github.com/Munto-dev/chat-document/blob/development/api/swagger.yaml

https://github.com/Munto-dev/chat-document/blob/development/api/socket-events.md
...
- **재연결 시 동기화 프로세스:기준점 (Sync Cursor):**
...
- **데이터베이스 스키마**: [ERD 문서](https://www.notion.so/munto/erd.md) 참조
- **실시간 통신 이벤트**: [Socket.IO Events 문서](https://www.notion.so/munto/socket-events.md) 참조
...
| **버전** | **일자** | **변경자** | **변경 내용** |
| --- | --- | --- | --- |
| v1.0.0 | 25.11.11 | 김범진 | 최초 작성 |
| v1.0.1 |  | 김범진 |  |
| v1.0.2 | 26.04.03 | 김범진 |   1) kafka → Redis Streams + Push Worker 변경
2.1 시스템 아키텍처 다이어그램
2.2 전체 시스템 구성
3.1.1 하드웨어 환경
3.1.2 소프트웨어 환경
...
7.8.7 메시지 검색 기능
2) 메시지 읽음 처리 구조 단순화
ERD 수정([https://github.com/Munto-dev/chat-document/pull/3/changes](https://github.com/Munto-dev/chat-document/pull/3/changes)) |
