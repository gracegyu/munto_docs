...
Project Description : 기존 Sendbird 기반 채팅 서비스를 자체 구축 채팅 서버(NestJS + Socket.IO + PostgreSQL + Redis)로 전환하기 위한 인프라를 신규 구성한다. 데이팅 DM을 1순위로 전환하고, 이후 소셜링/클럽/챌린지/문토 DM까지 단계적으로 마이그레이션하여 Sendbird 계약을 완전 해지한다. 채팅 서버는 문토 백엔드의 사용자 ID 체계(문토 userId)를 기반으로 인증을 처리하며, AWS ECS Fargate 위에서 독립 MSA로 운영한다.
...
| --- | --- | --- |
| **방식** | `prod-munto-redis-2` 재사용 | 채팅 전용 ElastiCache Redis 클러스터 신규 생성 
**`*cache.t4g.small`, Primary-Replica (Cluster Mode Disabled), 노드 2개 (Primary 1 + Replica 1)*** |
...
신규 ALB 생성. ECS 서비스와 함께 신규 구성. 
**Sticky Session 불필요** (munto_chat_sdk에서 `transports: ['websocket']` WebSocket Only로 설정되어 있어 HTTP Long Polling 단계가 없으므로 세션 불일치 문제 미발생)
**ALB Idle Timeout 300초로 설정 (**Socket.IO pingInterval 25초 기준 정상 동작하나, 모바일 네트워크 지연 등을 고려한 안전 마진 확보) |
...
3. Route53 `chat.munto.kr` A 레코드를 신규 ALB로 교체
4. 기존 `munto-be-chat-prod` ECS 서비스 종료
...
| v1.0.1 | 26.04.06 | 김범진 | Redis 구성 방식 명시 (Primary-Replica, Cluster Mode Disabled). ALB Idle Timeout 300초 설정 추가. Sticky Session 불필요로 변경 (WebSocket Only 채택 확인)
인프라 전환 순서 변경 |
