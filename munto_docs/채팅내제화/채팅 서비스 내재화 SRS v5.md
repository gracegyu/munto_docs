...
- **muntoUserId**: 채팅 서버·dating-backend·chat-notification이 공통으로 사용하는 사용자 식별자. 통합 JWT의 `sub` 클레임 값이자 dating DB `User.id`(FK 통합 구조)와 동일.
...
기존에는 모든 요구사항을 일괄 P1로 분류했으나, V1~~V4 전환 단계 도입에 따라 7장 각 기능을 **전환 단계 태그(`[V1]~~[V4]`) + P1~P3 우선순위**로 재분류한다. 기능별 우선순위·단계 태깅의 구체 작업은 후속 차수에서 7장과 함께 정리한다.
...
**V2+ 델타 동기화:** 위 재연결 갭 채우기를 *모든 방·백그라운드*로 확장한 형태다(`sync(lastMessageId)`로 다수 방을 일괄 동기화, 방목록 unread 일괄 갱신). V2 SDK에 추가되므로 V1 SDK 앱과 공존하며, 서버 `sync` API는 구버전 SDK가 호출하지 않아도 깨지지 않게 하위호환을 보장한다(2.8). 버전별 아키텍처 다이어그램은 §2.1 참조.
...
| **V1**     | 데이팅 DM 전환 (Closer 신규 + 임베드 데이팅 마이그레이션) + **백오피스 운영 필수 최소**(신고·차단·모니터링·메시지 삭제 API) | 2~4주   | 파일럿, 멀티 클라이언트. 백오피스 API는 기존 문토 백오피스가 소비 |
...
| 항목                   | 시작값                         | 비고                               |
| -------------------- | --------------------------- | -------------------------------- |
| 채팅 서버 task 스펙        | **1 vCPU / 2 GB**           | 실부하 대비 충분 + 재연결 버스트 헤드룸          |
| 채팅 서버 task 수         | **2 (Multi-AZ)**            | HA 목적(부하 아님)                     |
...
연결 수용량은 **task 스펙(vCPU·메모리)에 비례**하며 고정값이 아니다. 실제 운영 동시 접속은 수백 수준(§5.1)이므로 Task 수는 부하가 아니라 **HA(최소 2 task)** 기준으로 정한다.
...
**V1 개통 필수 범위 (운영 필수 최소):** 데이팅은 어뷰징·신고 리스크가 커서 아래는 V1 개통과 동시에 제공한다 — 신고 검토·**사용자 차단**(7.8.4), **채팅 내역 조회·모니터링**(7.8.2), **유해 메시지 삭제**(7.8.6), 접근 권한·**감사 로그**(7.8.1). 나머지(통계·금칙어 관리 UI·메시지 검색·자동 탐지)는 V2+로 미룬다.
