...
-- moim_type: SOCIALING | CLUB | CHALLENGE  (GROUP 전용)
...
CREATE TABLE cached_rooms (
    ...
    moim_type        TEXT,                       -- moim_type: SOCIALING | CLUB | CHALLENGE (GROUP 전용, 서버 chat_rooms.moim_type)
    moim_id          INTEGER,                    -- 모임 ID (GROUP 전용, 서버 chat_rooms.moim_id)
    name             TEXT,                       -- 채팅방 이름
    ...
    unread_count     INTEGER NOT NULL DEFAULT 0, -- 읽지 않은 메시지 수 (로컬 전용, 서버 last_read_message_id 기반 계산값)
    cached_at        BIGINT  NOT NULL            -- 로컬 캐싱 시간 (로컬 전용)
);

-- 목록 탭 필터(전체/DM/그룹, 그룹 세부 모임)용
CREATE INDEX idx_cached_rooms_type ON cached_rooms(type);
CREATE INDEX idx_cached_rooms_moim ON cached_rooms(moim_type, moim_id);
```
...
| `moim_type`, `moim_id` | 그룹 채팅방 전용. 서버 `chat_rooms.moim_type`/`moim_id`와 동일 (목록 탭/모임 필터용) |
| `last_message` | 목록 미리보기용 텍스트. 서버에는 없는 로컬 전용 필드 |
| `unread_count` | 서버 `last_read_message_id` 기반으로 계산하여 저장 (로컬 캐시 스냅샷) |
...
| `cached_rooms` | `chat_rooms` | `id`, `type`, `moim_type`, `moim_id`, `last_message_at` 동일 기준 |
...
V1 앱이 현장에 남은 채 V2~V4 스키마가 배포되므로(SRS §2.8 하위 호환성), V1부터 진화 가능한 스키마로 설계한다.
...
| **V2** | 방별 동기화 커서(예: `room_sync_state(room_id, last_message_id, synced_at)`) | 델타 동기화 `sync()` (SRS §2.7.5, 저장 대상 §4.6.1) |
