...
**Phase 2 (Month 3-9): Level 2 개발 (모델 v1.0)**

- 데이터 분석 및 모델 설계 (Month 3-6)
- 핵심 2개 모델 개발: 친구 수락 확률, 이탈 확률 (Month 6-9)
- 모델 v1.0 적용 (숫자 피처만, XGBoost)

**Phase 3 (Month 9-15): Level 2 검증 및 고도화 (모델 v2.0, 필수)**

- 모델 v1.0 A/B 테스트 및 롤아웃 (Month 9-12)
- 목표: ARPU +15-20%
- Image Embedding 추가 개발 (Month 12-15, 필수)
- 모델 v2.0 적용 (Neural Network + Image)
- 목표: AUC 0.85+ (모델 v1.0 대비 +5-10%)

**Phase 4 (Month 15-21): Level 3 개발 및 적용**

- 전략 정의 및 Contextual Bandit 개발 (Month 15-18)
- A/B 테스트 및 롤아웃 (Month 18-21)
- 목표: ARPU +25-35% (총합)

**Phase 5 (Month 21+): 지속 개선**
...
#### 3.1.2.1 모델 v1.0: Image 없이 시작 (간단, 권장 - 초기 적용)
...
#### 3.1.2.2 모델 v2.0: Image Embedding 추가 (중간, 필수 - 중기 적용)

**1. Image 전처리:** 크기: 224×224 (표준 CNN 입력 크기)

- ResNet, EfficientNet 사전학습 모델 사용
- 가로/세로 무관: Center crop + resize
- 컬러 유지 (RGB 3채널)

**2. Image → 중간 표현 (Embedding):** 사전학습 모델로 이미지를 숫자 벡터로 변환:

- ResNet-50 사용
- 마지막 FC layer 제거
- 중간 출력: 512차원 벡터 (이것은 Output이 아님!)

예: 사진 (1080×1920, RGB) → Resize to 224×224 → ResNet-50 (중간 표현 추출) → 512차원 벡터 [0.23, -0.45, 0.67, ...] → 이것은 "사진의 특징을 숫자로 표현한 것" → 아직 예측 결과 아님!
...
**모델 v2.0 (Image 포함) 적용 시: Neural Network 권장**
...
**실제 상황**: 사용자 프로필 사진은 3~6장 (가변)이며 순서가 있음

**옵션 1: 대표 이미지만 사용** (✅ 필수, Month 12-13, 모델 v2.0 초기)

```python
# 첫 번째 사진만 임베딩
user_image_embedding = resnet50(profile_photos[0])  # 512차원
```
...
**옵션 2: 평균 풀링 (Mean Pooling)** (✅ 권장, Month 14, 모델 v2.0 개선, 예상 효과: +1-2%)

```python
# 모든 사진 임베딩 후 평균
embeddings = [resnet50(photo) for photo in profile_photos]  # 3~6개
user_image_embedding = np.mean(embeddings, axis=0)  # 512차원
```
...
**옵션 3: 가중 평균 (Weighted Pooling)** (✅ 권장, Month 15, 모델 v2.0 고급, 예상 효과: +2-3% 추가)

```python
# 순서 기반 가중 평균
weights = [0.4, 0.3, 0.15, 0.1, 0.03, 0.02]  # 6장 기준
weights = weights[:len(profile_photos)]  # 실제 사진 수만큼
weights = weights / sum(weights)  # 정규화

embeddings = [resnet50(photo) for photo in profile_photos]
user_image_embedding = sum(w * emb for w, emb in zip(weights, embeddings))  # 512차원
```

장점:

- 순서 정보 반영 (첫 사진 40% 가중치)
- 모든 사진 활용
- Input 차원 고정
...
**옵션 4: 연결 + 제로 패딩 (Concatenation)** (❌ 비추천, AI 초보자에게 너무 복잡)

```python
# 최대 6장 고정, 부족하면 zero padding
max_photos = 6
embeddings = [resnet50(photo) for photo in profile_photos]

# 부족하면 제로 패딩
while len(embeddings) < max_photos:
    embeddings.append(np.zeros(512))

user_image_embedding = np.concatenate(embeddings)  # 3072차원 (512 × 6)
```
...
- Input 차원 폭발 (512 → 3072)
- 모델 복잡도 급증
- 학습 데이터 훨씬 더 필요
- 오버피팅 위험

**옵션 5: 어텐션 메커니즘 (Attention)** (❌ 비추천, 모델 v3.0 수준, Phase 5+ 장기 과제)

```python
# Transformer 스타일, 중요한 사진에 자동 집중
embeddings = [resnet50(photo) for photo in profile_photos]

# Attention 학습 (복잡)
attention_weights = attention_model(embeddings)  # 학습된 가중치
user_image_embedding = sum(w * emb for w, emb in zip(attention_weights, embeddings))
```
...
```python
# 각 사진 임베딩 미리 계산 후 Redis 저장
for idx, photo in enumerate(profile_photos):
    cache_key = f"image_emb:{user_id}:{idx}"
    embedding = resnet50(photo)
...
for idx in range(num_photos):
    cache_key = f"image_emb:{user_id}:{idx}"
    emb_bytes = redis.get(cache_key)
    embeddings.append(np.frombuffer(emb_bytes))
...
if method == "first":
    ...
    weights = [0.4, 0.3, 0.15, 0.1, 0.03, 0.02][:len(embeddings)]
...
| 접근      | Input                      | 모델           | 이유                   |
| --------- | -------------------------- | -------------- | ---------------------- |
| 모델 v1.0 | 숫자만 (50차원)            | XGBoost        | 정형 데이터 최적, 빠름 |
| 모델 v2.0 | 숫자+Image+Text (1330차원) | Neural Network | 복잡한 상호작용 학습   |
...
#### 3.1.2.3 모델 v3.0: End-to-End Multi-modal (고급, 연구 단계 - 장기 적용)

**핵심 차이**: 모델 v2.0는 "사전학습 모델로 임베딩 추출 → NN", 모델 v3.0은 "Image와 Text를 직접 입력 → CNN+BERT+예측 모델을 한 번에 학습"
...
### 3.1.4 모델 v2.0 상세: 어떻게 통합하나?

모델 v2.0를 적용할 때 Image + Text + 숫자를 통합하는 구체적 방법:

```python
# 모델 v2.0의 구현 (Early Fusion 방식)
...
**Phase 2 (Month 6-9): 모델 v1.0 - Text/숫자만** (필수)
...
**Phase 3 (Month 12-15): 모델 v2.0 - Image Embedding 추가** (필수)

```
Image → ResNet-50 → 512차원 임베딩
Text → Sentence-BERT → 768차원 임베딩
통합: Concatenation
모델: Neural Network (권장)
성능: AUC 0.85+ (모델 v1.0 대비 +5-10%)
기간: 3개월
...
**Phase 5+ (장기): 모델 v3.0 - End-to-End Multi-modal** (선택, 장기 과제)
...
1. **Phase 2 (모델 v1.0)**: Text/숫자 피처만 (필수)
   - 가장 빠르고 효과적
   - 빠른 검증 가능
2. **Phase 3 (모델 v2.0)**: Image Embedding 추가 (필수)
   - 사진이 데이팅 앱의 핵심 요소
   - 예상 개선: +5-10%
   - 모델 v1.0로 검증 후 바로 진행
3. **Phase 5+ (모델 v3.0)**: End-to-End Multi-modal (선택)
   - 장기 과제
   - 우선순위 낮음
   - 투자 대비 효과 미미
...
```
사용자 선택에 영향을 주는 요소:
1. 프로필 사진 (60-70%)  ← 가장 중요
2. 나이, 거리 (20-25%)     ← 기본 필터
3. MBTI, 관심사 (5-10%)    ← 보조 정보
4. 활동성, 수락률 (5%)
...
개발 전략:
- Phase 2 (모델 v1.0): Image 없이 빠른 검증 (3개월)
- Phase 3 (모델 v2.0): Image 추가 필수 (3개월)
- 이유: 초보자는 단계적 접근 필요
- 최종: 사진 포함 모델 완성
```
...
### 3.1.7 이미지 처리 기술 상세 (Phase 3 이후 참고)
...
- End-to-End Multi-modal (모델 v3.0)
- 장기 과제로 검토
- 투자 대비 효과 낮음

**핵심**: 모델 v1.0 → 모델 v2.0 단계적 진행이 필수입니다. AI 초보자는 XGBoost로 빠른 성공 경험 후, Image를 추가하는 것이 안전하고 효과적입니다. 프로필 사진 원본은 Phase 1부터 반드시 저장해야 합니다.

---

### 3.1.10 중요: 프로필 사진 데이터의 필수성
...
### 3.1.11 확장성 문제 및 해결 방안 (Scalability Challenge)
...
Phase 3-4 (Month 9-21):
- Level 1 가중치 고정 유지
- Level 3 개발 및 적용
- 4개 박스 중요도는 Level 3가 자동 학습 중

Phase 5+ (Month 21+, 선택적):
...
| ---------------- | ---------------- | --------------------- | ------ | ---------------- | ----------- | --------- |
| 모델 v1.0 (간접) | Level 3만 학습   | ~30개 (θ 가중치)      | 낮음   | 주간 자동        | 높음        | ✅ 권장   |
| 모델 v2.0 (직접) | Level 1 + 3 학습 | ~50개 (박스 내부 + θ) | 높음   | 매번 전체 재학습 | 중간        | ⚠️ 비추천 |
...
1. **Phase 1-4** (Month 0-21): Level 1 가중치 고정

   - 이유: 단순성, 빠른 배포, 충분한 성능
   - Level 3가 4개 박스의 최적 조합을 자동 학습

2. **Phase 5+** (Month 21+, 선택): Level 1 내부 가중치 최적화
...
### 초기 단계 (Phase 1-2, Month 0-9): 데이터 수집 + Level 2 기본 모델 (모델 v1.0)
...
1. **Level 1** (Traditional) 완전 구현
   ...
   - 기본 필터링 (거리, 나이, 활동성)
2. **Level 2 핵심 모델** 개발 (모델 v1.0: 숫자 피처만, XGBoost)
   - 친구 수락 확률 예측 (최우선)
   - 이탈 확률 예측
   - 모델: XGBoost
3. **데이터 로깅** 완전 구현
...
### 중기 단계 (Phase 3, Month 9-15): Level 2 검증 및 고도화 (모델 v2.0, 필수)

**목표**: 모델 v1.0 검증 및 Image 추가 개발

**구현 내용**:

1. **모델 v1.0 A/B 테스트** (Month 9-12)
   - 10% → 100% 점진적 롤아웃
   - ARPU +15-20% 검증
2. **모델 v2.0 개발** (Month 12-15, 필수)
   - Image Embedding 추가 (ResNet-50)
   - Neural Network 학습
   - 추론 파이프라인 고도화
3. **데이터 로깅** 확장
   - 프로필 사진 원본 저장 (이미 Phase 1부터 수행)
   - Image Embedding 캐싱

**산출물**:

- Level 2 모델 2개 (고도화, 모델 v2.0)
- AUC 0.85+ 달성

**목표 성과**: ARPU +20-25% (모델 v1.0 대비 추가 +5%)

---

### 후기 단계 (Phase 4, Month 15-21): Level 3 개발 + 수익 최적화
...
2. **통합 시스템**
   - Level 1 + 2 + 3 완전 통합
   - 자동 재학습 파이프라인
3. **Two-Tower 모델** 도입 (선택)
   - 확장성 확보 (100만 사용자 대응)

**목표 성과**: ARPU +25-35% (베이스라인 대비 총합)

---

### 지속 개선 (Phase 5+, Month 21+): 안정화 및 고도화
...
1. **자동화** 완성
   ...
   - 성능 모니터링 및 자동 조정
2. **고도화** (선택, 장기 과제)
   - 모델 v3.0 (End-to-End Multi-modal) 연구
   - 강화학습 전환 검토
   - 추가 Level 2 모델
...
**Phase 3 (Level 2 모델 v2.0 추가) 개선**:

```
모델 v1.0 대비:
- ARPU: 14,400원 → 15,840원 (+10%)
- AUC: 0.78 → 0.86 (+10%)
- 추론 시간: 15ms → 50ms
...
**Phase 4 (Level 3 추가) 개선**:

```
Level 2 대비:
- ARPU: 15,840원 → 19,008원 (+20%)
