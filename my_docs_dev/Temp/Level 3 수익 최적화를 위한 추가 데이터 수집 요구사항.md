...
데이팅 추천 시스템은 21개월에 걸쳐 단계적으로 발전하며, 각 단계마다 **추가** 로그가 필요합니다:

- **Phase 1** (Month 0-3): Level 1 전통적 박스 시스템 → 기본 로그
- **Phase 2** (Month 3-9): Level 2 개발 (모델 v1.0) → Level 2 로그 추가
- **Phase 3** (Month 9-15): Level 2 검증 및 고도화 (모델 v2.0, 필수)
- **Phase 4** (Month 15-21): Level 3 수익 최적화 추가 → Level 3 로그 추가
- **Phase 5** (Month 21+): 지속 개선

**로그는 누적됩니다**: Phase 4에서는 기본 로그 + Level 2 로그 + Level 3 로그를 **모두** 남깁니다.

이 문서는 각 Phase에서 **추가로** 필요한 로그를 명확히 구분하여 정리합니다.

**참고**: 전체 개발 로드맵은 [데이팅 서비스 AI 접목 개발 로드맵](./데이팅%20서비스%20AI%20접목%20개발%20로드맵.md) 참조
...
Phase 2-3 (Month 3-15):
├─ 기본 로그 (유지)
└─ Level 2 추가 로그
   - PREDICTION (예측 결과 저장)
   - MODEL_PERFORMANCE (예측 정확도 추적)
   - Phase 2: 모델 v1.0 (숫자 피처만)
   - Phase 3: 모델 v2.0 (Image 추가, 필수)
...
└─ Level 3 추가 로그
   - EXPOSURE (전략 정보)
   - REWARD_AGGREGATION (수익 계산)
   - STRATEGY_PERFORMANCE (전략 성과)
   - Context 벡터
```

**핵심**: Phase가 진행될수록 로그가 **추가**됩니다. 기존 로그는 계속 유지됩니다.

### 2.2 단계별 로그 비교표

| 로그 종류                  | Phase 1<br>(Month 0-3) | Phase 2-3<br>(Month 3-12) | Phase 4+<br>(Month 12+) | 비고                 |
| -------------------------- | ---------------------- | ------------------------- | ----------------------- | -------------------- |
| **기본 이벤트**            | ✅ 필수                | ✅ 유지                   | ✅ 유지                 | VIEW, LIKE, MATCH 등 |
| VIEW, LIKE, FRIEND_REQUEST | ✅                     | ✅                        | ✅                      | 모든 Phase 필수      |
| MATCH, MESSAGE, PAYMENT    | ✅                     | ✅                        | ✅                      | 모든 Phase 필수      |
| **Level 2 추가**           | -                      | ✅ 추가                   | ✅ 유지                 | AI 예측 모델용       |
| PREDICTION                 | ✗                      | ✅                        | ✅                      | 예측 결과 검증       |
...
- **Phase 2-3** (Month 3-15): 기본 + Level 2 로그 (11개 이벤트 타입)
  - Phase 2: 모델 v1.0 (숫자만, XGBoost)
  - Phase 3: 모델 v2.0 (Image 추가, Neural Network, 필수)
- **Phase 4+** (Month 15+): 기본 + Level 2 + Level 3 로그 (14개 이벤트 타입)

**로그는 누적**: Phase 4에서는 1, 2, 3 모든 로그를 남깁니다.

---

## 3. Phase 1 (Month 0-3): 기본 로그
...
✅ Phase 1 및 향후 Phase의 기반 데이터로 충분

---

## 4. Phase 2-3 (Month 3-15): Level 2 추가 로그

**Phase 구성**:

- Phase 2 (Month 3-9): 모델 v1.0 개발 (숫자 피처만, XGBoost)
- Phase 3 (Month 9-15): 모델 v1.0 검증 + 모델 v2.0 개발 (Image 추가, 필수)
...
## 5. Phase 4+ (Month 15+): Level 3 추가 로그
...
**Month 15-18: Level 3 로그 수집** (학습 전)
