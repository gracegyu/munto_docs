...
데이팅 서비스 출시부터 AI 완전 적용까지 **21개월 로드맵**입니다. Cursor, Claude 같은 AI 코딩 도구를 적극 활용하여 **데이터 사이언티스트 1명**으로 전체 개발을 진행합니다. 서비스 시작 후 3개월간 데이터 수집에 집중하고, 가장 중요한 2개 모델(친구 수락 확률, 이탈 확률)을 우선 개발합니다. Level 2 모델 개발은 모델 v1.0(숫자 피처만) → 모델 v2.0(Image 추가, 필수) 순으로 진행하며, Level 2 안정화 후 Level 3 수익 최적화를 시작합니다. 총 투자 2.2억원으로 연 3.6억원 추가 수익 창출, 회수 기간 8개월 예상입니다.
...
```
Month 0-3:   Phase 1 - 기반 구축 (Level 1 배포, 데이터 수집)
Month 3-9:   Phase 2 - Level 2 개발 (모델 v1.0: 숫자만, XGBoost)
Month 9-15:  Phase 3 - Level 2 검증 및 고도화 (모델 v2.0: Image 추가, 필수)
Month 15-21: Phase 4 - Level 3 개발 및 적용 (Bandit, A/B 테스트)
Month 21+:   Phase 5 - 지속 개선 (자동 재학습, 전략 최적화)
```
...
## 3. Phase 2: Level 2 개발 (Month 3-9)

### 3.0 전체 개요

Phase 2는 두 단계로 나뉩니다:

- Month 3-6: 데이터 분석 및 모델 설계
- Month 6-9: 모델 개발 및 완성

### 3.1 Month 3-6: 데이터 분석 및 준비
...
## 4. Phase 3: Level 2 검증 및 고도화 (Month 9-15)

### 4.1 전체 개요

Phase 3는 두 단계로 나뉩니다:

- Month 9-12: 모델 v1.0 A/B 테스트 및 롤아웃
- Month 12-15: 모델 v2.0 Image 추가 개발 (필수)

### 4.2 Month 9-12: 모델 v1.0 검증

**목표**:

- ✅ Level 2 모델 (모델 v1.0) A/B 테스트 실행
- ✅ 프로덕션 점진적 롤아웃
- ✅ 성과 측정 및 개선
- ✅ Image 데이터 준비 (Phase 1부터 저장된 사진 확인)

### 4.3 Month 12-15: 모델 v2.0 개발 (필수)

**목표**:

- ✅ Image Embedding 추가 개발 (ResNet-50)
- ✅ Neural Network 모델 학습
- ✅ 모델 v2.0 A/B 테스트 및 롤아웃
- ✅ AUC 0.85+ 달성

### 4.4 주요 작업 (Month 9-12: 모델 v1.0 검증)
...
Week 1-2: 10% 트래픽 A/B 테스트
  - Control (50%): Level 1만
  - Treatment (50%): Level 1 + 2 (모델 v1.0: 친구 수락 + 이탈 예측)
...
Week 3-4: 모델 v2.0 개발 착수 (필수)
  - 프로필 사진 데이터 확인 (Phase 1부터 저장됨)
  - Image 전처리 파이프라인 설계
  - ResNet-50 환경 준비
```

### 4.5 주요 작업 (Month 12-15: 모델 v2.0 개발, 필수)

**Month 12-13: Image Embedding 파이프라인 구축**

```
Week 1-2: Image 전처리 구현
  - 224×224 resize 및 정규화
  - Center crop 적용
  - RGB 3채널 유지

Week 3-4: ResNet-50 Embedding 생성
  - 사전학습 모델 로드
  - 모든 사진 512차원 임베딩 추출
  - Redis 캐싱 구현
```

**Month 13-14: Neural Network 모델 개발**

```
Week 1-2: 학습 데이터 준비
  - 숫자 피처 (50차원) + Image (512차원) + Text (768차원)
  - 총 1330차원 Input 생성
  - Train/Val/Test 분할

Week 3-4: Neural Network 학습
  - 아키텍처: Dense(512) → Dense(256) → Dense(128) → Output
  - 하이퍼파라미터 튜닝
  - 목표: AUC 0.85+ (모델 v1.0 대비 +5-10%)
```

**Month 14-15: 모델 v2.0 배포 및 검증**

```
Week 1-2: SageMaker 배포
  - GPU Endpoint 설정 (ml.g4dn.xlarge)
  - 추론 API 수정
  - 로드 테스트

Week 3-4: 오프라인 검증 및 성과 확인
  - 실제 데이터로 정확도 검증
  - AUC 0.85+ 달성 확인
  - 추론 시간 < 80ms 확인
```
...
**기술 지표** (Month 9-12, 모델 v1.0):

- [ ] 모델 정확도: AUC 0.75-0.80
- [ ] 추론 API 가용성 99.9%
- [ ] P95 응답시간 < 50ms

**비즈니스 지표** (Month 12-15, 모델 v2.0, 추가):

- [ ] ARPU: 모델 v1.0 대비 +5-10% 추가
- [ ] 베이스라인 대비 총 +20-30%
- [ ] 리텐션 유지

**기술 지표** (Month 12-15, 모델 v2.0):

- [ ] 모델 정확도: AUC 0.85+ (모델 v1.0 대비 +5-10%)
- [ ] 추론 API 가용성 99.9%
- [ ] P95 응답시간 < 80ms (GPU 사용)

### 4.7 산출물

**Month 9-12 (모델 v1.0)**:

- A/B 테스트 결과 리포트 (모델 v1.0)
- Level 2 성과 분석 리포트
- 프로덕션 모델 (친구 수락 + 이탈, 모델 v1.0)
- 모니터링 대시보드

**Month 12-15 (모델 v2.0, 필수)**:

- Image Embedding 파이프라인
- Neural Network 모델 (친구 수락 + 이탈, 모델 v2.0)
- 모델 v2.0 검증 리포트
- Level 3 개발 계획서

### 4.8 의사결정 포인트 (Month 15 말)

**Level 3 개발 진행 확인**:

```
진행 조건:
✅ 모델 v2.0 성과 달성 (AUC 0.85+, ARPU +25-30%)
✅ 시스템 안정성 확보
✅ 추가 투자 가능 (인력, 예산)

다음 단계:
→ Level 3 개발 착수 (Month 15-21)
   - 예상 추가 효과: +20-30%
   - 기간: 6개월

이유: 모델 v1.0 → 모델 v2.0 완료, 이제 수익 최적화(Level 3)로 진행
```

---

## 5. Phase 4: Level 3 개발 및 적용 (Month 15-21)

### 5.0 전체 개요

Phase 4는 두 단계로 나뉩니다:

- Month 15-18: Level 3 개발 (전략 정의, Bandit 구현)
- Month 18-21: Level 3 적용 (A/B 테스트, 롤아웃)

### 5.1 Month 15-18: Level 3 개발
...
목표: 최소 2개월 (Month 16-17) 데이터 수집
```

**Month 17: 보상 계산 시스템 구축**
...
- [ ] ARPU: +50% 이상 (vs Phase 0 베이스라인)
- [ ] ARPU: +20% 이상 (vs Phase 3 Level 2)
- [ ] LTV: +40% 이상
- [ ] 30일 리텐션: -5% 이내
- [ ] 매칭 성공률: -10% 이내
...
| Phase   | 기간        | DS (전담) | 주요 작업                                     | AI 도구 효과                 |
| ------- | ----------- | --------- | --------------------------------------------- | ---------------------------- |
| Phase 1 | Month 0-3   | 0.3명     | Level 1 배포, 데이터 수집, 모니터링           | 분석 코드 자동 생성          |
| Phase 2 | Month 3-9   | 1명       | 데이터 분석, 모델 설계, Level 2 모델 2개 개발 | EDA, 모델 코드 자동 생성     |
| Phase 3 | Month 9-12  | 1명       | Level 2 A/B 테스트, 롤아웃, 성과 분석         | 통계 분석, 리포트 자동       |
| Phase 4 | Month 12-18 | 1명       | Level 3 개발, A/B 테스트, 롤아웃, 자동화      | Bandit, 파이프라인 코드 생성 |
| Phase 5 | Month 18+   | 0.5명     | 자동 재학습, 모니터링, 전략 최적화            | 거의 완전 자동화             |
...
| 항목                     | Phase 1     | Phase 2     | Phase 3     | Phase 4       | Phase 5     |
| ------------------------ | ----------- | ----------- | ----------- | ------------- | ----------- |
| 인건비 (DS 1명)          | 830만원     | 830만원     | 830만원     | 830만원       | 420만원     |
| SageMaker                | 13만원      | 50만원      | 65만원      | 100만원       | 65만원      |
| S3 스토리지              | 7만원       | 13만원      | 13만원      | 20만원        | 20만원      |
| DynamoDB                 | 13만원      | 20만원      | 26만원      | 40만원        | 40만원      |
| AI 도구 (Cursor, Claude) | 7만원       | 13만원      | 13만원      | 13만원        | 7만원       |
| 기타 (CloudWatch 등)     | 7만원       | 10만원      | 13만원      | 20만원        | 13만원      |
| **총계**                 | **877만원** | **936만원** | **960만원** | **1,023만원** | **565만원** |

**21개월 예산 총합** (추정):

- DS 인건비: 1억원/년 × 1.75년 = 1.75억원
- AWS 인프라 (SageMaker, S3, DynamoDB): 2,800만원
- AI 도구 (Cursor Pro, Claude API): 1,700만원
- 기타 (CloudWatch, 교육 등): 1,200만원
- **총 예산**: **약 2.2억원**

**비교**:

- AI 도구 없을 때: 약 4.4억원 (DS 2명 + MLOps 1명 × 21개월)
- AI 도구 활용: 약 2.2억원
- **절감**: 약 2.2억원 (50%)
...
```
Month 3 (Phase 1 → 2):
- 데이터 충분? → Yes: Level 2 개발 진행, No: 3개월 연장

Month 9 (Phase 2 → 3):
- Level 2 모델 성능 목표 달성? → Yes: A/B 테스트 진행, No: 재개발

Month 12 (Phase 3 → 4):
- Level 2 효과 검증 (+15%+)? → Yes: Level 3 개발, No: Level 2 고도화

Month 18 (Phase 4 → 5):
- Level 3 목표 달성 (+25%+)? → Yes: 유지보수 모드, No: 재조정
```
...
```
서비스 출시 (M0) - Phase 1 시작
    ↓
  [3개월 데이터 수집]
    ↓
데이터 분석 시작 (M3) - Phase 2 시작
    ↓
  [6개월 Level 2 개발]
    ↓
Level 2 모델 완성 (M9) - Phase 3 시작
    ↓
  [3개월 A/B 테스트 및 롤아웃]
    ↓
Level 2 프로덕션 (M12) - Phase 4 시작
    ↓
  [6개월 Level 3 개발 및 적용]
    ↓
전체 시스템 완성 (M18) - Phase 5 시작
    ↓
  [지속적 개선]
```
...
- Month 15: Level 2 프로덕션 (ARPU +20%, 모델 v1.0+2)
- Month 21: Level 3 프로덕션 (ARPU +25%)

**이상적 목표** (적극적):

- Month 12: Level 2 모델 v1.0 완료 (ARPU +15%)
- Month 15: Level 2 모델 v2.0 완료 (ARPU +25%)
- Month 21: Level 3 프로덕션 (ARPU +35%)
...
| 마일스톤 | 시점     | Phase | 내용              | 성공 기준        |
| -------- | -------- | ----- | ----------------- | ---------------- |
| M1       | Month 3  | 1→2   | 데이터 수집 완료  | 100,000+ 이벤트  |
| M2       | Month 9  | 2→3   | Level 2 모델 완성 | AUC 0.85+        |
| M3       | Month 12 | 3→4   | Level 2 효과 검증 | ARPU +15%        |
| M4       | Month 18 | 4→5   | Level 3 효과 검증 | ARPU +25% (총합) |
...
| 항목                      | 금액           | 비고                      |
| ------------------------- | -------------- | ------------------------- |
| 인건비 (DS 1명 × 21개월)  | 1.75억원       | 연 1억원                  |
| AWS 인프라 (21개월)       | 2,800만원      | SageMaker, S3, DynamoDB   |
| AI 도구 라이선스 (21개월) | 1,700만원      | Cursor Pro, Claude API 등 |
| 기타 (교육, 컨퍼런스 등)  | 1,200만원      | -                         |
| **총계**                  | **약 2.2억원** | -                         |

**비교**:

- AI 도구 없을 때: 약 4.4억원 (DS 2명 + MLOps 1명)
- AI 도구 활용: 약 2.2억원
- **절감**: 약 2.2억원 (50%)
...
2. **Month 3-9**: Level 2 핵심 2개 개발 (모델 v1.0)
3. **Month 9-15**: Level 2 고도화 (모델 v2.0, 필수) 및 성과 검증
4. **Month 15-21**: Level 3 개발 및 적용
5. **Month 21+**: 안정화 및 지속 개선
...
- Month 15: ARPU +20-30% (Level 2, 모델 v1.0+2)
- Month 21: ARPU +25-40% (Level 3 추가)
- ROI: 1.6배 (첫 해), 4배+ (2년차)
- 회수 기간: 8개월
